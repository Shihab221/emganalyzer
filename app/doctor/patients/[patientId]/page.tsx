            ...
            {sessions.map((s) => (
              <li key={s.id}>
                <div className="glass-card overflow-hidden rounded-xl">
                  {/* flex-wrap + basis-full on result panel stacks content below each session row */}
                  <div className="flex flex-wrap items-center">
                    <Link
                      href={`/doctor/patients/${patientId}/session/${s.id}`}
                      className="flex flex-wrap flex-1 min-w-[200px] items-center justify-between gap-3 px-4 py-3 hover:bg-slate-50/80 dark:hover:bg-slate-800/40"
                    >
                      ...
                      <span className="inline-flex items-center gap-1 text-sm text-red-500 font-medium shrink-0">
